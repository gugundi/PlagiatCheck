...
def libSignatures(docsDict:dict):
    sigDict = {}

    print("Creating library signatures")
    for key in tqdm(docsDict.keys()):
        tokens = tokenize(docsDict[key])
        minh = MinHash(num_perm=k)
        for to in tokens:
            minh.update(to.encode('utf8'))
        sigDict[key] = list(minh.hashvalues)
    return sigDict
...
test(rebuildSigDict=True,debug=False)



#hashes = libSignatures(docs)

#for key,val in hashes.items():
 #   print("Key: {:30s} | Number of hashvalues: {}".format(key,len(val.hashvalues)))
